...
revert x Hx.
elim (Pos.to_nat n); clear n; [|intros n IHn]; intros x Hx.
{ now simpl; case F.toX; [|intro r; simpl; rewrite Rmult_1_r]. }
simpl.
rewrite IHn.
{ rewrite F.div2_correct; [|easy|].
  { rewrite Xdiv_split.
    case F.toX; [easy|]; intro rx.
    simpl; unfold Xinv'; rewrite is_zero_false; [|easy].
    now simpl; rewrite Rmult_assoc. }
  revert Hx; apply Rle_trans.
  unfold Rdiv; rewrite Rmult_1_l.
  rewrite <-(Rmult_1_r (/ 256)) at 1.
  apply Rmult_le_compat_l; [|apply pow_R1_Rle]; lra. }
unfold F.toR; rewrite F.div2_correct; [|easy|].
{ revert Hx; unfold F.toR; case F.toX; [|intro rx].
  { simpl; change R0 with 0%R; rewrite Rabs_R0.
    intro H; exfalso; apply (Rlt_irrefl 0); revert H; apply Rlt_le_trans.
    do 2 (apply Rmult_lt_0_compat; [lra|]).
    apply pow_lt; lra. }
  rewrite Xdiv_split; unfold Xinv'.
  simpl.
  intro Hx.
  rewrite is_zero_false; [|easy].
  simpl.
  rewrite Rabs_mult, (Rabs_pos_eq (/ 2)); lra. }
revert Hx; apply Rle_trans.
unfold Rdiv; rewrite Rmult_1_l.
rewrite <-(Rmult_1_r (/ 256)) at 1.
apply Rmult_le_compat_l; [|apply pow_R1_Rle]; lra.
...
Proof.
change c1_2 with (iter_pos F.div2 1 c1); rewrite c1_n_correct.
{ unfold c1; rewrite F.fromZ_correct by easy; simpl; apply f_equal; lra. }
unfold F.toR, c1; rewrite F.fromZ_correct by easy; simpl; rewrite Rabs_R1; lra.
Qed.

Lemma c1_8_correct : F.toX c1_8 = Xreal (/ 256).
Proof.
change c1_8 with (iter_pos F.div2 8 c1); rewrite c1_n_correct.
{ unfold c1; rewrite F.fromZ_correct by easy; simpl; apply f_equal; lra. }
unfold F.toR, c1; rewrite F.fromZ_correct by easy; simpl; rewrite Rabs_R1; lra.
Qed.
...
clear prec p.
intros prec.
assert (H: forall p, (2 <= p <= 256)%Z ->
    contains (I.convert (atan_fast0 prec (I.inv prec (I.fromZ_small p)))) (Xreal (atan (/ IZR p)))).
  intros p Hp.
  ...
  apply IZR_lt.
  now apply Z.lt_le_trans with 2%Z.
  replace (Xreal (/ IZR p)) with (Xinv (Xreal (IZR p))).
  apply I.inv_correct.
  apply I.fromZ_small_correct.
    rewrite Z.abs_eq.
    apply Hp.
    now apply Z.le_trans with 2%Z.
  unfold Xinv'.
  simpl.
...
Proof.
intros prec x Rx Bx.
unfold atan_fastP, c1_2, c1, c2.
rewrite F'.lt_correct with (2 := Rx).
2: {
  unfold F.toR.
  now fold c1; fold c1_2; rewrite c1_2_correct. }
unfold F.toR at 1.
fold c1; fold c1_2; rewrite c1_2_correct.
simpl Rlt_bool.
assert (Ix: contains (I.convert (I.bnd x x)) (Xreal (toR x))).
{ rewrite I.bnd_correct; [|apply I.valid_lb_real|apply I.valid_ub_real];
    rewrite Rx; [|easy..].
...
rewrite F'.lt_correct with (2 := Rx);
  [|now unfold F.toR; rewrite F.fromZ_correct; [..|lia]].
unfold F.toR at 1.
...
rewrite H, atan_0.
simpl.
rewrite F.zero_correct, F'.valid_lb_zero, F'.valid_ub_zero.
split ; apply Rle_refl.
(* pos *)
...
assert (Hexit : forall k powi divi,
  ...
  rewrite I.bnd_correct.
  2: { now apply I.valid_lb_real; rewrite F.zero_correct. }
  2: { apply I.valid_ub_upper.
    exists (x ^ k * x / INR (k + 1 + 1))%R.
    now apply J.div_correct; [apply J.mul_correct|]. }
  rewrite F.zero_correct, I.upper_correct.
  2: { exists (x ^ k * x / INR (k + 1 + 1))%R.
    now apply J.div_correct; [apply J.mul_correct|]. }
...
assert (H: forall prec xi x,
  ...
  rewrite I.upper_correct in Hxu; [|now exists x].
  destruct (I.convert xi) as [|l [|u]] ; try easy.
  revert Hxu.
  simpl.
  unfold thre, c1_p_c1_8, c1.
  elim (F.add_DN_correct (F.PtoP 52) (F.fromZ 1) c1_8).
  2: apply F'.valid_lb_real ; now rewrite F.real_correct, F.fromZ_correct.
  2: apply F'.valid_lb_real ; now rewrite F.real_correct, c1_8_correct.
  intros Va.
...
rewrite F'.valid_lb_zero, F'.valid_ub_nan.
refine (conj _ I).
...
case_eq (F.classify x); intro Cx; [|easy..|];
  [case_eq (F.toX x); [easy|]; intros rx Hrx _
  |case_eq (F.toX x); [|easy]; intros Hx _].
{ (* x real *)
  unfold Xcmp, c1.
  case Rcompare_spec; intro Hrx0; [easy..|].
  rewrite F.cmp_correct, Cx, Hrx.
  generalize (F.classify_correct (F.fromZ 1)).
  rewrite F.real_correct, F.fromZ_correct by easy.
  ...
  { (* x < 1 *)
    ...
    intros prec'.
    rewrite <- (Rinv_involutive rx); [|lra].
    unfold Xln, Xln'.
    rewrite ln_Rinv; [|now apply Rinv_0_lt_compat].
    rewrite is_positive_true; [|now do 2 apply Rinv_0_lt_compat].
    apply J.neg_correct.
    ...
    now apply I.valid_ub_real; rewrite Hrx. }
  { (* x = 1 *)
    intros ->.
    unfold Xln, Xln'.
    rewrite is_positive_true; [|lra].
...
assert (Ix: contains (I.convert xi) (Xreal (toR x))).
  ...
  2: now apply I.valid_lb_real; rewrite Rx.
...
assert (Hexit: forall k powi divi,
  ...
  assert (Hne : not_empty (I.convert (I.div prec (I.mul prec powi x2i) facti))).
  { exists ((F.toR x ^ (2 * k) * (F.toR x * F.toR x)) / INR (fact (2 * (k + 1))))%R.
    now apply J.div_correct; [apply J.mul_correct; [|apply J.sqr_correct]|]. }
  rewrite I.bnd_correct.
  2: now apply I.valid_lb_real; rewrite F.zero_correct.
  2: now apply I.valid_ub_upper.
  rewrite F.zero_correct, I.upper_correct; [|easy].
...
refine (_ (I.fromZ_small_correct 1 _) (I.fromZ_small_correct 2 _) (I.fromZ_small_correct 3 _)) ; [|easy..].
fold i1 i2 i3.
generalize i1 i2 i3.
...
{ intros.
  exact (H x Hxr Hx H0). }
rewrite F'.le_correct; try (unfold F.toR; rewrite c1_2_correct); [simpl|easy..].
intros Hxhalf.
refine (_ (IHnb (F.div2 x) _ _)).
{ destruct (sin_cos_reduce prec (F.div2 x) nb) as (ss, ci).
  fold (F.toR x).
  replace (F.toR x) with (2 * (toR (F.div2 x)))%R.
  { generalize (toR (F.div2 x)).
    revert Hxr Hxhalf; clear; intros Hxr Hxhalf.
    intros hx (Hc, Hs).
    split.
    { (* - cos *)
    ...
    { apply Rmult_le_pos_pos.
      ...
      exact Hs. }
    exact (proj2 (H _ Hc)). }
  unfold toR; rewrite F.div2_correct; [|easy|].
  { rewrite Xdiv_split; simpl; unfold Xinv'; rewrite is_zero_false; [|easy].
    rewrite Hxr; simpl; lra. }
...
assert (Ix: contains (I.convert xi) (Xreal (toR x))).
  ...
  rewrite I.bnd_correct, Rx.
  split ; apply Rle_refl.
  apply F'.valid_lb_real ; now rewrite F.real_correct, Rx.
  apply F'.valid_ub_real ; now rewrite F.real_correct, Rx.
apply J.mul_correct with (2 := Ix).
apply J.sub_correct.
now apply I.fromZ_small_correct.
set (x2i := I.sqr prec xi).
...
assert (Hexit : forall k powi divi,
  ...
  intros k powi divi Hpow Hdiv.
  assert (Hne : not_empty (I.convert (I.div prec (I.mul prec powi x2i) divi))).
  { exists ((F.toR x ^ (2 * k) * (F.toR x * F.toR x)) / INR (fact (2 * (k + 1) + 1)))%R.
    now apply J.div_correct; [apply J.mul_correct; [|apply J.sqr_correct]|]. }
  rewrite I.bnd_correct.
  2: now apply I.valid_lb_real; rewrite F.zero_correct.
  2: now apply I.valid_ub_upper.
  rewrite F.zero_correct, I.upper_correct by easy.
...
refine (_ (I.fromZ_small_correct 1 _) (I.fromZ_small_correct 6 _) (I.fromZ_small_correct 4 _)) ; [|easy..].
fold i1 i6 i4.
generalize i1 i6 i4.
intros powi divi facti.
change 1%R with (pow (toR x) (2 * 0)).
...
Proof.
intros prec x Hxr Hx0.
unfold sin_fastP, cm1, c1.
case_eq (F'.le' x c1_2) ; intros Hx.
...
assert (Hexit : forall k powi divi,
  ...
  intros k powi divi Hpow Hdiv.
  assert (Hne : not_empty (I.convert (I.div prec (I.mul prec powi xi) divi))).
  { exists ((F.toR x ^ (k + 1) * F.toR x)/ INR (fact (k + 2)))%R.
    now apply J.div_correct; [apply J.mul_correct|]. }
  rewrite I.bnd_correct.
  2: now apply I.valid_lb_real; rewrite F.zero_correct.
  2: now apply I.valid_ub_upper.
...
refine (_ Ix (I.fromZ_small_correct 2 _) (I.fromZ_small_correct 3 _)) ; [|easy..].
fold i2 i3.
generalize i2 i3.
...
{ intros prec x Hx1 Hx2 Hx3.
  ...
  revert Hx3.
  rewrite Hx1.
  rewrite c1_8_correct.
  lra. }
intros prec x Hx H0.
unfold expn_reduce.
case_eq (F'.le' x c1_8) ; intros Hx'.
{ (* . no reduction *)
  now apply H. }
(* . reduction *)
clear Hx'.
...
{ (* nb = 0 *)
  ...
  rewrite F'.valid_ub_real by now rewrite F.real_correct, F.fromZ_correct.
...
{ rewrite (Rabs_pos_eq _ (Rlt_le _ _ H0)).
  ...
  unfold Rdiv; rewrite Rmult_1_l.
  simpl.
  now case Rcompare_spec; [easy..|]; intros H' _; apply Rlt_le. }
assert (toR (F.div2 x) = toR x * /2)%R.
{ unfold toR; rewrite F.div2_correct; [|easy..].
  rewrite Xdiv_split; simpl; unfold Xinv'; rewrite is_zero_false; [|easy].
  rewrite Hx; simpl; lra. }
replace (toR x) with (toR (F.div2 x) + toR (F.div2 x))%R.
{ rewrite Ropp_plus_distr.
  rewrite exp_plus.
  change (Xreal (exp (- toR (F.div2 x)) * exp (- toR (F.div2 x))))
    with (Xsqr (Xreal (exp (- toR (F.div2 x))))).
  apply I.sqr_correct.
  apply IHnb.
  { unfold F.toR; rewrite F.div2_correct; [|easy..].
    rewrite Xdiv_split; simpl; unfold Xinv'; rewrite is_zero_false; [|easy].
...
unfold c1.
rewrite I.bnd_correct.
2: apply I.valid_lb_real ; now rewrite F.fromZ_correct.
2: now apply I.valid_ub_nan.
split.
rewrite F.fromZ_correct by easy.
simpl.
rewrite <- exp_0.
